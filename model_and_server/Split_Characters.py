...
lowerModel =load_model('Models/Lower_alexnet_model/best_val_loss.hdf5')
coreModel = load_model('Models/Core_AlexNet_model/best_val_loss.hdf5')
upperModel = load_model('Models/Upper_alexNet_model/best_val_acc.hdf5')
...
def Split(Words):
 ...
 # Assuming white pixels are represented as 255
            # plt.imshow(simg)
            # plt.show()
            seg, con = modifier_segmentation(simg, base)
            print("sege")
            print(seg)
        ...
        print(f"the confidence of the word is ${np.average(confidence)}")
        # words += word
        words += (word + ' ')
    ...
    try:
        ...
        if len(new_keys) >2 :
            ...
            if np.sum(upper_modifier == 255) > 50:
                upper_pred,con  = predit_uppper(upper_modifier)
        ...
        for c in cores_imgs:
            ...
            if np.sum(c == 255) > 100:
                # plt.imshow(c)
                # plt.show()
                core_pred, con = predit_core(c)
                confidence.append(con)
                cores.append(core_pred)
        ...
        if lower_modifier.shape[0]>=min_seg and lower_modifier.shape[1]>=min_seg and np.sum(c == 255) > 50: 
            print("this is a herll")
            lowerMod,con = predit_lower(lower_modifier)
            confidence.append(con)
            result += [lower_modifiers[lowerMod]]
...
def predit_uppper(image):
    ...
    predictions = upperModel.predict(x)
    print(predictions)
...
def predit_lower(image):
    ...
    # plt.show()
    x = np.array([thresh]).reshape(-1, 32, 32, 1) / 255.0
    predictions = lowerModel.predict(x)
    print(predictions)
    y = np.argmax(predictions)
    confidence = predictions[0, y]  # Get the confidence/probability of the predicted class
   
    print("lower Modifiers",y)
    return y,confidence

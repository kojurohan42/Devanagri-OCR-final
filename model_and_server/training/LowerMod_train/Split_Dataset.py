...
for Class in tqdm(
    df.iloc[:, 0], unit_scale=True, miniters=1, desc="Splitting Dataset "
):
    ...
    if len(Files) == 2000:
        for File in Files[:200]:
            shutil.copy(
                os.path.join("Dataset", str(Class), File),
                os.path.join("Split Dataset", "Validation", str(Class), File),
            )
        for File in Files[200:]:
            shutil.copy(
                os.path.join("Dataset", str(Class), File),
                os.path.join("Split Dataset", "Train", str(Class), File),
            )
    elif len(Files) == 600:
...
shutil.copy(
    os.path.join("Dataset", "Reference.csv"),
